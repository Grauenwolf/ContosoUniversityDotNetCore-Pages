...
public class DetailsTests
{
	...
	public async Task Should_get_details()
	{
		...
		var studentId = await (new Create(scope.Db, scope.Mapper)).Handle(command);
		...
		var details = await (new Details(scope.Db, scope.Mapper)).Handle(new Details.Query { Id = studentId });
