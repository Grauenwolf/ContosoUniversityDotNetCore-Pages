...
public class IndexTests
{
	...
	public async Task Should_get_list_instructor_with_details()
	{
		...
		var instructor1Id = await (new CreateEdit(scope.Db, scope.Mapper)).Handle(new CreateEdit.Command
		{
			FirstMidName = "George",
			LastName = "Costanza",
		...
		var instructor2Id = await (new CreateEdit(scope.Db, scope.Mapper)).Handle(new CreateEdit.Command
		{
		...
		var result = await (new Index(scope.Db, scope.Mapper)).Handle(new Index.Query { Id = instructor1Id, CourseId = english101.Id });
